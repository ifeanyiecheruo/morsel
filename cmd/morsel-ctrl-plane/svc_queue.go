...
func queueWriteError(ctx context.Context, w http.ResponseWriter, code int, errCode, message string) {
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(code)
	if err := json.NewEncoder(w).Encode(map[string]string{"error": errCode, "message": message}); err != nil {
...
func (h *queueHandler) createQueue(w http.ResponseWriter, r *http.Request) {
	ns, err := h.identify(r)
	if err != nil {
		queueWriteError(r.Context(), w, http.StatusUnauthorized, "unauthorized", err.Error())
		return
	}
	if err := h.queueFor(ns).CreateQueue(r.Context(), r.PathValue("name")); err != nil {
		queueWriteError(r.Context(), w, http.StatusInternalServerError, "internal_error", err.Error())
		return
	}
...
func (h *queueHandler) deleteQueue(w http.ResponseWriter, r *http.Request) {
	...
	if err := h.queueFor(ns).DeleteQueue(r.Context(), r.PathValue("name")); errors.Is(err, queue.ErrQueueNotFound) {
		queueWriteError(r.Context(), w, http.StatusNotFound, "queue_not_found", "queue does not exist")
		return
	} else if err != nil {
		queueWriteError(r.Context(), w, http.StatusInternalServerError, "internal_error", err.Error())
		return
...
func (h *queueHandler) enqueue(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		queueWriteError(r.Context(), w, http.StatusBadRequest, "bad_request", err.Error())
		return
	}
	q := h.queueFor(senderNS)
	if err := q.Enqueue(r.Context(), r.PathValue("name"), body, senderNS); errors.Is(err, queue.ErrQuotaExceeded) {
		used, usageErr := q.Usage(r.Context())
		if usageErr != nil {
			ctxlog.From(r.Context()).Warn("get queue usage", "err", usageErr)
		}
		w.Header().Set("Content-Type", "application/json")
		...
		if err := json.NewEncoder(w).Encode(map[string]any{
			...
			ctxlog.From(r.Context()).Warn("write error response", "err", err)
		}
		return
...
func (h *queueHandler) setQuota(w http.ResponseWriter, r *http.Request) {
	if !h.checkInternalToken(r) {
		queueWriteError(r.Context(), w, http.StatusUnauthorized, "unauthorized", "invalid internal token")
		return
	}
