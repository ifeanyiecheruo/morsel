...
func (h *cliHandler) AppDeploy(ctx context.Context, prof *Profile) error {
	...
	deployClient, err := client.New(prof.APIURL, session.Token)
	if err != nil {
...
func buildPushDeploy(ctx context.Context, client *client.Client, org, repo string, cfg morselConfig, registry, shortSHA string) (string, error) {
...
func pollOperation(ctx context.Context, client *client.Client, org, repo, name, opID string) error {
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
