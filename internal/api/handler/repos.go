...
func (h *Handler) ListRepos(ctx context.Context, params oas.ListReposParams) (oas.ListReposRes, error) {
	...
	if claims.Role == tokens.RoleOperator && params.All.Or(false) {
		...
		out := make(oas.ListReposOKApplicationJSON, len(repos))
	...
	if claims.Role == tokens.RoleDeveloper {
		...
		out := make(oas.ListReposOKApplicationJSON, 0)
		return &out, nil
	}
	...
	out := oas.ListReposOKApplicationJSON{dbRepoToOAS(repo, count)}
	return &out, nil
}

func (h *Handler) GetRepo(ctx context.Context, params oas.GetRepoParams) (oas.GetRepoRes, error) {
	...
	repo, err := h.store.GetRepo(ctx, slug)
	if errors.Is(err, sql.ErrNoRows) {
		return &oas.GetRepoNotFound{Error: oas.ErrorDetail{
			Code:    "not_found",
...
func (h *Handler) SyncRepo(ctx context.Context, req *oas.SyncRepoReq, params oas.SyncRepoParams) (oas.SyncRepoRes, error) {
	if err := checkRepoAccess(ctx, params.Org, params.Repo); err != nil {
		return nil, err
	}
	slug := repoSlug(params.Org, params.Repo)
